...
void find_collinear_straightforward(point2D* p, int n) {
    ...
    for (int i = 0; i < n - 2; i++) {
        ...
        for (int j = i + 1; j < n - 1; j++) {
            ...
            double slope = calc_slope((double)p1.x, (double)p1.y, (double)p2.x, (double)p2.y);
            double yIntercept = calc_y_intercept((double)p1.x, (double)p1.y, slope);
            ...
            if (slope < DBL_MAX) {
                ...
                for (int k = j + 1; k < n; k++) {
                    ...
                    if (is_colinear((double)p3.x, (double)p3.y, slope, yIntercept)) {
                        ncol++;
//                        printf("Points: %i, %i, %i\n", i, j, k);
                    }
                }
...
void find_collinear_improved(point2D* p, int n) {
    ...
    for (int i = 0; i < n - 1; i++) {
        ...
        for (int j = i + 1; j < n; j++) {
            ...
            struct LnSegment ln;
            ln.slope = calc_slope((double)p1.x, (double)p1.y, (double)p2.x, (double)p2.y);
            ln.x1 = p1.x;
            ln.y1 = p1.y;
            ln.x2 = p2.x;
            ln.y2 = p2.y;
